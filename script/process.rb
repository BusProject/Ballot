...
def addCandidate obj
  ...
  if !obj['Electoral District'].nil? && !obj['Office Name'].nil? # A general check - mostly to stop blank rows from proceeding  
    ...
    obj['Electoral District'] = obj['State']+obj['Electoral District'] if obj['Electoral District'].slice(0,2) != obj['State']  # Appends the state onto the geography if not there
