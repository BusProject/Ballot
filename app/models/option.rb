class Option < ActiveRecord::Base
  ...
  has_many :feedback, :conditions => [ "length(comment) > 1 AND length(flag)- length(replace( flag,',','') ) < ? AND approved = ?", 2, true ], :order => ['cached_votes_up - cached_votes_down DESC'], :limit => 3 do
